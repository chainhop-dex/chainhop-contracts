...
describe('1inchCodec', () => {
    ...
    it('should decode and override fillOrderRFQ calldata', async function () {
        ...
        let data = ethers.utils.defaultAbiCoder.encode(
            ['tuple(uint256 info, address makerAsset, address takerAsset, address maker, address allowedSender, uint256 makingAmount, uint256 takingAmount) order', 'bytes signature', 'uint256 makingAmount', 'uint256 takingAmount'],
    ...
    it('should decode and override swap calldata', async function () {
        ...
        let data = ethers.utils.defaultAbiCoder.encode(
            ['address caller', 'tuple(address srcToken, address dstToken, address srcReceiver, address dstReceiver, uint256 amount, uint256 minReturnAmount, uint256 flags, bytes permit) desc', 'bytes data'],
            [user, desc, '0x00']
        );
        data = data.slice(2); // strip 0x
