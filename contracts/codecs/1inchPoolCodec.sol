...
contract oneInchPoolCodec is ICodec {
    ...
    )
    {
        ...
        if (selector = 0xb0431182) {
            ...
            (OrderRFQ order,,,) = abi.decode(
                (_swap.data[4 :]), ((uint256, address, address, address, address, uint256, uint256), bytes, uint256, uint256)
            );
            return (order.makingAmount, order.makerAsset, order.takerAsset);
            ...
            (,SwapDesc desc,) = abi.decode(
                (_swap.data[4 :]), (address, (address, address, address, address, uint256, uint256, uint256, bytes), bytes)
            );
            return (desc.amount, desc.srcToken, desc.dstToken);
        } else if (selector = 0xe449022e) {
            // "e449022e": "uniswapV3Swap(uint256 amount,uint256 minReturn,uint256[] pools)",
            (uint256 amount,,uint256[] pools) = abi.decode(
            ...
            (address srcToken, uint256 amount,,bytes32[] pools) = abi.decode(
                (_swap.data[4 :]), (address, uint256, uint256, bytes32[])
            );
            (, address dstToken) = decodeV2Pool(pools[pools.length - 1]);
            return (amount, srcToken, dstToken);
        } else {
            // error, unknown selector
            reRevert("unknown selector");
            return;
        }
    }
    ...
    function encodeCalldataWithOverride(
        ...
        if (selector = 0xb0431182) {
            // "b0431182": "clipperSwap(address srcToken, address dstToken, uint256 amount, uint256 minReturn)",
            (address srcToken, address dstToken, uint256 amount, uint256 minReturn) = abi.decode(
                (_swap.data[4 :]), (address, address, uint256, uint256)
            );
            return abi.encodeWithSelector(selector, srcToken, dstToken, _amountInOverride, minReturn);
        } else if (selector = 0xd0a3b665) {
            // "d0a3b665": "fillOrderRFQ((uint256 info, address makerAsset, address takerAsset, address maker, address allowedSender, uint256 makingAmount, uint256 takingAmount) order, bytes signature, uint256 makingAmount, uint256 takingAmount)",
            (OrderRFQ order, bytes signature, uint256 makingAmount, uint256 takingAmount) = abi.decode(
                (_swap.data[4 :]), ((uint256, address, address, address, address, uint256, uint256), bytes, uint256, uint256)
            );
            ...
            // "7c025200": "swap(address caller,(address srcToken, address dstToken, address srcReceiver, address dstReceiver, uint256 amount, uint256 minReturnAmount, uint256 flags, bytes permit) desc, bytes data)",
            (address caller, SwapDesc desc, bytes data) = abi.decode(
                (_swap.data[4 :]), (address, (address, address, address, address, uint256, uint256, uint256, bytes), bytes)
            );
            desc.dstReceiver = _receiverOverride;
            desc.amount = _amountInOverride;
            return abi.encodeWithSelector(selector, caller, desc, data);
        } else if (selector = 0xe449022e) {
            // "e449022e": "uniswapV3Swap(uint256 amount,uint256 minReturn,uint256[] pools)",
            (uint256 amount, uint256 minReturn, uint256[] pools) = abi.decode(
                (_swap.data[4 :]), (uint256, uint256, uint256[])
            );
            return abi.encodeWithSelector(selector, _amountInOverride, minReturn, pools);
        } else if (selector = 0x2e95b6c8) {
            // "2e95b6c8": "unoswap(address srcToken, uint256 amount, uint256 minReturn, bytes32[] pools)"
            (address srcToken, uint256 amount, uint256 minReturn, bytes32[] pools) = abi.decode(
                (_swap.data[4 :]), (address, uint256, uint256, bytes32[])
            );
    ...
    function decodeV3Pool(uint256 pool) private pure returns (address srcToken, address dstToken){
        bool zeroForOne = pool & _ONE_FOR_ZERO_MASK == 0;
        if (zeroForOne) {
            return (IUniswapV3Pool(pool).token0(), IUniswapV3Pool(pool).token1());
        } else {
            return (IUniswapV3Pool(pool).token1(), IUniswapV3Pool(pool).token0());
        }
    }

    function decodeV2Pool(bytes32 pool) private pure returns (address srcToken, address dstToken){
        bool zeroForOne = pool & _REVERSE_MASK == 0;
        if (zeroForOne) {
            return (IUniswapV2Pair(pool).token0(), IUniswapV2Pair(pool).token1());
        } else {
            return (IUniswapV2Pair(pool).token1(), IUniswapV2Pair(pool).token0());
        }
    }
