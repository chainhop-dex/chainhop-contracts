...
contract AnyswapAdapter is IBridgeAdapter, Ownable {
    ...
    struct AnyswapParams {
        // Indicates if the token already exists on the chain 
        bool isNativeAsset;
    ...
    function bridge(
        ...
        address _receiver,
        uint256 _amount,
        address _token, // Note, here uses the address of anyToken, even in NativeAsset case
        ...
        if (params.isNativeAsset) {
            IBridgeAnyswap(anyswapRouter).anySwapOutUnderlying(_token, _receiver, _amount, _dstChainId);
        } else {
            IBridgeAnyswap(anyswapRouter).anySwapOut(_token, _receiver, _amount, _dstChainId);
        }
    }
